...
/**
 * List tickets in NH Sprint 37 currently assigned to New Onshore (Huzefa, Gururaj, Jack, Allan).
 */
...
async function main() {
  ...
  console.log('\n🔎 Fetching issues in NH Sprint 37...');
  ...
  console.log('📋 TICKETS CURRENTLY ASSIGNED TO NEW ONSHORE (Huzefa, Gururaj, Jack, Allan)');
  console.log('='.repeat(70));

  if (assigned.length === 0) {
    console.log('\n   No tickets are currently assigned to New Onshore.\n');
    return;
  }

  console.log(`\n   Found ${assigned.length} ticket(s):\n`);
  assigned.forEach((t) => {
    const assignee = getDisplayName(t.fields?.assignee);
    console.log(`   ${t.key} | ${t.fields?.status?.name || '?'} | ${t.fields?.issuetype?.name || '?'} | ${assignee}`);
    console.log(`      ${(t.fields?.summary || '').slice(0, 65)}`);
    console.log('');
  });
}
