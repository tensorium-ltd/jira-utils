...
Auto-generated and updated by `generate-sprint-sheets.js` with the following structure:
- **Row 1**: Date headers (working days only)
- **Row 2**: Story Points Committed / Planned (S-curve distributed)
- **Row 3**: Actual Story Points (daily completed points from JIRA) - **AUTO-FILLED**
- **Row 4**: Cumulative Planned (cumulative sum of committed points)
- **Row 5**: Cumulative Actual (cumulative sum of actual points) - **AUTO-CALCULATED**
- **Row 6**: Variance (Cumulative Planned - Cumulative Actual) - **AUTO-CALCULATED**
- **Row 7**: Burndown (Remaining work)

**Note**: Rows marked as **AUTO-FILLED** or **AUTO-CALCULATED** are automatically populated when running `npm run generate-sheets` with JIRA credentials configured.
...
Generates individual sprint sheets from the main Excel plan and updates the Progress sheet with actual daily completions from JIRA.
...
**Usage**:
```bash
# Requires JIRA credentials for Progress sheet updates
export JIRA_EMAIL="your.email@company.com"
export JIRA_API_TOKEN="your_jira_api_token"
